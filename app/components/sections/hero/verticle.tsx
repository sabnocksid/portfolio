              ...
              >
                <div className="flex flex-col items-center text-sm font-bold leading-4">
                  {link.label.split("").map((char, idx) => (
                    <span key={idx} className="rotate-90 hidden md:block">{char}</span>
                  ))}
                </div>

<div className="my-3 border border-primary-deep p-1 rounded-full flex items-center justify-center 
